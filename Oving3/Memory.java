...
public class Memory {
	...
	public Process checkMemory(long clock) {
		if(!memoryQueue.isEmpty()) {
			Process nextProcess = (Process)memoryQueue.getNext();
                        if (Simulator.info) System.out.println("memory needed: " + nextProcess.getMemoryNeeded() + "  available: "+getFreeMemory()+ "/" + getMemorySize());
    ...
    public void processCompleted(Process p) {
        if (Simulator.info) System.out.println("freememory: " + freeMemory);
        freeMemory += p.getMemoryNeeded();
        if (Simulator.info) System.out.println("freememory after process completed: " + freeMemory);
    }
    ...
    {
        return memoryQueue.getQueueLength();
    }
